...
class Captura:
    ...
    def getCaptura(self):
        responseCaptura = requests.get("https://www.datos.gov.co/resource/m5pi-7cau.json")
        self.dataJson = responseCaptura.json()
        for ind in range(len(self.dataJson)):
            print("Proveedor -->"+str(self.dataJson[ind]["proveedor"]))
